...
namespace doublebox_nonplanar
{
    ...
    )
    {
        // Construct Integrals
        #if doublebox_nonplanar_contour_deformation
            std::vector<nested_series_t<sum_t>> integral_doublebox_nonplanar_integral = doublebox_nonplanar_integral::make_integral(real_parameters,complex_parameters,integrator,number_of_presamples,deformation_parameters_maximum,deformation_parameters_minimum,deformation_parameters_decrease_factor);
        #else
            std::vector<nested_series_t<sum_t>> integral_doublebox_nonplanar_integral = doublebox_nonplanar_integral::make_integral(real_parameters,complex_parameters,integrator);
        #endif
        ...
        for (unsigned int amp_idx = 0; amp_idx < number_of_amplitudes; ++amp_idx)
        {
            ...
            amplitude  = doublebox_nonplanar_integral::make_weighted_integral(real_parameters, complex_parameters, integral_doublebox_nonplanar_integral, amp_idx, lib_path);
    ...
    )
    {
        ...
        DYNAMIC_CAST_INTEGRATOR_KOROBOV_QMC(3,3)
    ...
    // secdecutil::integrators::Qmc
    INSTANTIATE_MAKE_AMPLITUDES_KOROBOV_QMC(3,3)
