...
#include "doublebox_nonplanar_integral_weighted_integral.hpp"

#define INTEGRAL_NAME doublebox_nonplanar
...
namespace doublebox_nonplanar_integral
{
    ...
    {
        assert(doublebox_nonplanar::requested_orders.size() == doublebox_nonplanar_integral::requested_orders.size());
        size_t number_of_regulators = doublebox_nonplanar::requested_orders.size();
        std::vector<int> orders; orders.reserve( doublebox_nonplanar::requested_orders.size() );
        for(size_t i = 0; i < number_of_regulators; ++i)
        {
            int order = (
                              doublebox_nonplanar::requested_orders.at(i) + 1
                            - doublebox_nonplanar_integral::lowest_coefficient_orders.at(amp_idx).at(i)
                            - doublebox_nonplanar_integral::lowest_orders.at(i)
                            - doublebox_nonplanar_integral::lowest_prefactor_orders.at(i)
                        );
    ...
    {
        std::string coeff_filename(lib_path + "/doublebox_nonplanar_integral_coefficient" + std::to_string(amp_idx) + ".txt");
...
namespace doublebox_nonplanar
{
    namespace doublebox_nonplanar_integral
    {
        ...
        struct WithCuda
        {
            using integrand_t = ::doublebox_nonplanar_integral::integrand_t;

            // Note: we define make_integrands with doublebox_nonplanar_contour_deformation
            // but call ::sub_integral_name::make_integrands with doublebox_nonplanar_integral_contour_deformation
            ...
            {
                    return ::doublebox_nonplanar_integral::make_integrands
                    (
                        real_parameters,
                        complex_parameters
            ...
            {
                using integrand_t = ::doublebox_nonplanar_integral::cuda_integrand_t;
                ...
                {
                        return ::doublebox_nonplanar_integral::make_cuda_integrands
                        (
                            real_parameters,
                            complex_parameters
                            #if doublebox_nonplanar_integral_contour_deformation
        ...
        // and return one with ::doublebox_nonplanar_integral::(cuda_)integrand_t instead
        #define INSTANTIATE_AMPLITUDE_INTEGRAL_NONE_QMC() \
            ...
            { \
                using amplitude_integrator_t = secdecutil::integrators::Qmc< \
                     ...
                     INTEGRAL_NAME::maximal_number_of_integration_variables, \
                     ::integrators::transforms::None::type, \
                     ::doublebox_nonplanar_integral::INTEGRAND_TYPE, \
                     secdecutil::integrators::void_template \
                >; \
        ...
        )
        {
            ...
            using integrand_return_t = doublebox_nonplanar::integrand_return_t;
            ...
            const std::function<sum_t(const integrand_t& integrand)> convert_integrands =
                [ integrator_ptr ] (const integrand_t& integrand) -> sum_t
                {
                    ...
                    integral_ptr->display_name = ::doublebox_nonplanar_integral::package_name + "_" + integrand.display_name;
        ...
        )
        {
            ...
            amplitude *= ::doublebox_nonplanar_integral::prefactor(real_parameters,complex_parameters)
                       * ::doublebox_nonplanar_integral::coefficient(real_parameters,complex_parameters,amp_idx,lib_path);
            return amplitude;
        }
